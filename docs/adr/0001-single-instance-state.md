...
- speaking-session handles and cleanup schedules;
...
- SignalR connections and classroom presence;
...
to Redis, add an Azure SignalR or Redis backplane, persist data-protection keys in Blob
Storage or Key Vault, and make cleanup jobs lease-based so only one instance performs
