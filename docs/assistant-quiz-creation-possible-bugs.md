...
| Analytics SQL completeness audit | Fixed in documentation | The audit now compares every completed invocation with its own `usage_debit` through a one-row correlated apply. It detects partially settled multi-invocation turns without letting a settled failed invocation or duplicate ledger rows hide the gap. Its diagnostic text says “missing usage settlement”; under the current billing policy a terminal usage settlement is a normal `usage_debit`. |
